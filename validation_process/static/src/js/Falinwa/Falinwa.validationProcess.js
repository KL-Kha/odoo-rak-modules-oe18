...
class validationProcess {
    ...
    constructor(container) {
        ...
        s.domain = container.domain;

        console.log("s........", s, container);
        // LOCAL VARIABLES
        s.isDisabled = false;
    ...
    async onDocumentChanged(id = false, model = false, view_type = false, force = false) {
        ...
        if (!s.execution) {
            ...
            console.log("...........", s.domain.prototype.stringToArray(s.process[s.config.processFields.startConditionDomain]));
            debugger;
            ...
            let triggeredProcessStart = await s.matchDomain(
                s.id,
                s.model,
                s.domain.prototype.stringToArray(s.process[s.config.processFields.startConditionDomain]),
                s.domain.prototype.stringToArray(s.process[s.config.processFields.filterDomain]));
    ...
    async getActiveRules(rulesToCheck = false) {
        ...
        for (var i = 0; i < rulesToCheck.length; i++) {
            ...
            let matched = await s.matchDomain(
                s.id,
                s.model,
                s.domain.prototype.stringToArray(rule[s.config.ruleFields.applyOnDomain]),
                s.domain.prototype.stringToArray(s.process[s.config.processFields.filterDomain])
            );
            if (matched) {
                rules.push(rule);
    ...
    async checkIfAnyRuleIsActive(rulesToCheck = false) {
        ...
        for (var i = 0; i < rulesToCheck.length; i++) {
            ...
            masterDomain = masterDomain.concat(s.domain.prototype.stringToArray(rule[s.config.ruleFields.applyOnDomain]))
        }
