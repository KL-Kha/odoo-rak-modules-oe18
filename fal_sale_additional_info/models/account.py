...
class AccountMove(models.Model):
    ...
    # Modification Majorbird DEC08 2022: remove compute and readonly as it cascades updates
    # on many models sale.order > account.move > account.payment > account.bank.statement.line
    # leads to big performance issues
    fal_sale_total_ordered_amount = fields.Monetary(
        string='Total ordered amount', store=True)
    fal_sale_total_amount_already_invoced = fields.Monetary(
        string='Total invoiced amount', store=True)
    fal_percentage_invoiced_ordered = fields.Float(
        string='% Invoiced / ordered', store=True)
