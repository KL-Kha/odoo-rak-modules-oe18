...
xhr.open("POST", "/web/create_and_edit_many2one/check_permission", false);  // false makes it synchronous
...
try {
    ...
    if (xhr.status === 200) {
        ...
        console.log("[create_and_edit_many2one] Failed to check permission:", xhr.status);
        hasPermission = false;
    }
} catch (error) {
    console.log("[create_and_edit_many2one] Failed to check permission:", error);
    hasPermission = false;
}
