...
class stock_card(models.Model):
    ...
    line_ids = fields.One2many('fal.stock.card.line', 'stock_card_id', string='Details')
    ...
    def action_calculate(self):
        ...
        for sc in self:
            ...
            sql = "select sum(quantity) from stock_move_line where product_id=%s \
                  and date < '%s' and location_dest_id IN %s \
                  and id IN %s \
                  and state='done' and picked = TRUE" % (
            ...
            sql_prod_qty_out = "select sum(quantity) from stock_move_line \
                where product_id=%s and date < '%s' and \
                location_id IN %s and state='done' and picked = TRUE" % (
